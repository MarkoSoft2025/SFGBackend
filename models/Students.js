...
module.exports = {
  ...
  getAuth: (username, password, callback) => {
    const query = 'SELECT * FROM tbl_student WHERE username = ? AND password = ?';
  ...
  create: (data, callback) => {
    const query = 'INSERT INTO tbl_student SET ?';
  ...
  getById: (id, callback) => {
    const query = 'SELECT * FROM tbl_student WHERE studentid = ?';
  ...
  delete: (id, callback) => {
    const query = 'DELETE FROM tbl_student WHERE studentid = ?';
